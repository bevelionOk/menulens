...
| 3.2 Schema + migration | Tables per architecture; generated SQL file in repo; migrate script idempotent | Hand-written SQL instead of generated = fine too, but it must be a *file*, not just `db push`. **Ratified done 2026-08-21** (session 7, spec `1-2-shared-contract-data-layer`, D22): Story 1.2 also carries the `shared` Zod contract (one base per entity, snake_case wire keys) and the repos' two read invariants; `bytea` via `customType`; migrate = explicit script, never at boot |
| 3.3 Intake endpoints | `POST /api/menus` accepts `{url}` or multipart file; validates type/size; creates `menus` row with status | Reject early with typed errors; no queue — request handles it (guardrail). **Ratified done 2026-08-22** (session 8, spec `1-3-persist-first-run-lifecycle-api`, D23): routes are `POST /api/runs` + `GET /api/runs/:id` (list → 3.1, artifact → 2.4); run born `processing`/`stage=null`, derived `interrupted` via one pure function used by the 409 gate and the read; transitions logged with `run_id` |
